...
class FileCheck:
    ...
    def get_file_times(self, filename):
        ...
        file_time = round(clip.duration/60,2)
        file_width = clip.duration
...
def main():
    ...
    datas = [[u'路径+名称', u'分钟（保留两位小数点）']]  # two-dimensional array
    #datas = [[u'FileName', u'Size', u'分钟（保留两位小数点）', u'Width', u'Height', u'CompressRatio']]  # two-dimensional array
    ...
    for f in files:
        ...
        print(u"FileName:{filename},Duration:{filetimes}"
		      .format(filename=f, filetimes=file_times))
        cell.append(f)
